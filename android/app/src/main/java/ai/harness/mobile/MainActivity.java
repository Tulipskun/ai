...
public class MainActivity extends Activity {
    ...
    void send(){String text=message.getText().toString().trim();if(text.isEmpty())return;io.execute(()->{try{if(session.isEmpty()){JSONObject sb=new JSONObject();sb.put("provider",provider.getText().toString().trim());sb.put("model",model.getText().toString().trim());session=request("POST","/v1/sessions",sb.toString()).getString("id");after=0;save();}JSONObject b=new JSONObject();b.put("message",text);String m=model.getText().toString().trim();if(!m.isEmpty())b.put("model",m);JSONObject o=request("POST","/v1/sessions/"+session+"/messages",b.toString());job=o.getString("job_id");after=0;add("you: "+text);message.post(()->message.setText(""));poll();}catch(Exception e){add("send: "+e.getMessage());}});}
