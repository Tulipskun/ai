...
func startDaemon(app string) error {
	root := filepath.Dir(app); stateDir := filepath.Join(root, ".ai")
	if err := os.MkdirAll(stateDir, 0o755); err != nil { return err }
	pidPath := filepath.Join(stateDir, "ai.pid")
	if data, err := os.ReadFile(pidPath); err == nil { if pid, err := strconv.Atoi(strings.TrimSpace(string(data))); err == nil && processAlive(pid) { return fmt.Errorf("ai is already running (pid %d)", pid) }; _ = os.Remove(pidPath) }
	logFile, err := os.OpenFile(filepath.Join(stateDir, "ai.log"), os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o644); if err != nil { return err }
	cmd := exec.Command(app, "daemon"); cmd.Dir=root; cmd.Env=os.Environ(); cmd.Stdin=nil; cmd.Stdout=logFile; cmd.Stderr=logFile; cmd.SysProcAttr=&syscall.SysProcAttr{Setsid:true}
	if err:=cmd.Start();err!=nil{_ = logFile.Close();return err};pid:=cmd.Process.Pid;if pid<=0{_ = logFile.Close();_=cmd.Process.Kill();_=cmd.Process.Release();return fmt.Errorf("failed to start ai daemon: invalid pid %d",pid)}
	if err:=os.WriteFile(pidPath,[]byte(strconv.Itoa(pid)+"\n"),0o644);err!=nil{_=cmd.Process.Kill();_=logFile.Close();_=cmd.Process.Release();return err};_ = logFile.Close();_ = cmd.Process.Release();fmt.Printf("[ai] started (pid %d)\n",pid);return nil
}

func stopDaemon(app string) error {
	root:=filepath.Dir(app);pidPath:=filepath.Join(root,".ai","ai.pid");data,err:=os.ReadFile(pidPath);if err!=nil{if os.IsNotExist(err){return nil};return err};pid,err:=strconv.Atoi(strings.TrimSpace(string(data)));if err!=nil||pid<=0{_ = os.Remove(pidPath);return nil};if !processAlive(pid){_ = os.Remove(pidPath);return nil}
	if err:=syscall.Kill(pid,syscall.SIGTERM);err!=nil&&!errors.Is(err,syscall.ESRCH){return err};deadline:=time.Now().Add(10*time.Second);for time.Now().Before(deadline){if !processAlive(pid){_ = os.Remove(pidPath);return nil};time.Sleep(100*time.Millisecond)};if processAlive(pid){_=syscall.Kill(pid,syscall.SIGKILL)};_=os.Remove(pidPath);return nil
}

func runDaemon() error { ctx,stop:=signal.NotifyContext(context.Background(),os.Interrupt,syscall.SIGTERM);defer stop();return run(ctx,false) }
func runCLI() error { app,err:=installedBinary();if err==nil{_ = os.Chdir(filepath.Dir(app))};ctx,stop:=signal.NotifyContext(context.Background(),syscall.SIGTERM);defer stop();return run(ctx,true) }

func run(ctx context.Context, cliOnly bool) error {
	providerConfigPath:=envOr("AI_PROVIDER_CONFIG",".config/provider.json");providerFile,err:=runtime.LoadProviderFile(providerConfigPath);if err!=nil{return err};rt,err:=runtime.Load(providerConfigPath);if err!=nil{return err};providerManager:=runtime.NewProviderManager(providerConfigPath,rt,providerFile);maxOutputTokens,err:=envInt("AI_MAX_OUTPUT_TOKENS");if err!=nil{return err};sessionDB:=envOr("AI_SESSION_DB",".data/sessions.db");providerID:=strings.TrimSpace(os.Getenv("AI_PROVIDER"));if providerID==""&&len(rt.ProviderConfigs)==1{providerID=string(rt.ProviderConfigs[0].ID)};modelID:=strings.TrimSpace(os.Getenv("AI_MODEL"));sessions:=runtime.NewSessionManagerWithProviders(sessionDB,sdk.SessionConfig{Provider:sdk.ProviderID(providerID),Model:modelID},rt.ProviderConfigs);defer sessions.Close();browserConfig,err:=runtime.LoadBrowserConfig();if err!=nil{return err};if browserConfig.Enabled{if err:=rt.StartBrowser(ctx,browserConfig);err!=nil{return fmt.Errorf("start browser: %w",err)};defer rt.CloseBrowser()};transportConfig,err:=transport.LoadConfig();if err!=nil{return err};if !cliOnly&&!transportConfig.DiscordEnabled{log.Printf("no Discord transport enabled; set DISCORD_BOT_TOKEN")};workspace:=envOr("AI_WORKSPACE",".");agent,err:=newAgent(rt.Client,workspace,rt.Browser,browserConfig.AllowPrivate);if err!=nil{return err};providerKeys:=make(map[sdk.ProviderID]*sdk.KeyPool,len(rt.ProviderConfigs));for _,provider:=range rt.ProviderConfigs{providerKeys[provider.ID]=provider.Keys}
