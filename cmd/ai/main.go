...
func run(ctx context.Context, cliOnly bool) error {
	...
	browserConfig,err:=runtime.LoadBrowserConfig(filepath.Join(state,runtime.DefaultBrowserConfigPath));if err!=nil{return err};if browserConfig.Enabled{if err:=rt.StartBrowser(ctx,browserConfig,state);err!=nil{log.Printf("browser unavailable, continuing without browser automation: %v",err)};defer rt.CloseBrowser()}
	...
	workspace,err:=resolveWorkspace();if err!=nil{return err};jobsPath:=filepath.Join(state,"data","jobs.json");agent,err:=newAgent(rt.Client,workspace,rt.Browser,browserConfig.AllowPrivate,jobsPath);if err!=nil{return err};providerKeys:=make(map[sdk.ProviderID]*sdk.KeyPool,len(rt.ProviderConfigs));for _,provider:=range rt.ProviderConfigs{providerKeys[provider.ID]=provider.Keys}
...
func resolveWorkspace()(string,error){raw:=strings.TrimSpace(os.Getenv("AI_WORKSPACE"));if raw==""{home,err:=os.UserHomeDir();if err!=nil||strings.TrimSpace(home)==""{return ".",nil};return home,nil};path:=expandHome(raw);if err:=os.MkdirAll(path,0o755);err!=nil{return "",fmt.Errorf("AI_WORKSPACE: %w",err)};return path,nil}
func expandHome(path string)string{if path=="~"{if home,err:=os.UserHomeDir();err==nil&&strings.TrimSpace(home)!=""{return home};return path};if strings.HasPrefix(path,"~/")||strings.HasPrefix(path,"~\\"){if home,err:=os.UserHomeDir();err==nil&&strings.TrimSpace(home)!=""{return filepath.Join(home,path[2:])}};return path}
