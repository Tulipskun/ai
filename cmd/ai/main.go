...
func startDaemon(app string) error {
	...
	stateDir := filepath.Join(state, ".ai"); if err := os.MkdirAll(stateDir, 0o755); err != nil { return err }
	...
	pid:=cmd.Process.Pid; if pid<=0 { _=logFile.Close(); _=cmd.Process.Kill(); _=cmd.Process.Release(); return fmt.Errorf("failed to start ai daemon: invalid pid %d",pid) }
...
func runCLI() error { state,err:=stateRoot();if err!=nil{return err};if err:=os.MkdirAll(state,0o755);err!=nil{return err};if err:=os.Chdir(state);err!=nil{return err};ctx,stop:=signal.NotifyContext(context.Background(),syscall.SIGINT,syscall.SIGTERM);defer stop();return run(ctx,true) }
func run(ctx context.Context, cliOnly bool) error {
	...
	inputConfigPath:=filepath.Join(state,transport.DefaultConfigPath);transportConfig,err:=transport.LoadConfig(inputConfigPath);if err!=nil{return err};if !cliOnly&&!transportConfig.DiscordEnabled{log.Printf("no Discord transport enabled; configure .config/input.json")}
	workspace:=envOr("AI_WORKSPACE", ".");agent,err:=newAgent(rt.Client,workspace,rt.Browser,browserConfig.AllowPrivate);if err!=nil{return err};providerKeys:=make(map[sdk.ProviderID]*sdk.KeyPool,len(rt.ProviderConfigs));for _,provider:=range rt.ProviderConfigs{providerKeys[provider.ID]=provider.Keys}
