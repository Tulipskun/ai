...
func run(ctx context.Context, cliOnly bool) error {
	state, err := stateRoot()
	if err != nil { return err }
	providerConfigPath := statePath(state, "AI_PROVIDER_CONFIG", ".config/provider.json")
	...
	sessionDB := statePath(state, "AI_SESSION_DB", ".data/sessions.db")
...
func statePath(state, envName, fallback string) string { value := envOr(envName, fallback); if filepath.IsAbs(value) { return filepath.Clean(value) }; return filepath.Join(state, value) }
