...
func run() error {
	...
	sessions := runtime.NewSessionManagerWithProviders(sessionDB, sdk.SessionConfig{}, rt.ProviderConfigs)
	defer sessions.Close()

	browserConfig, err := runtime.LoadBrowserConfig()
	if err != nil { return err }
	if browserConfig.Enabled {
		if err := rt.StartBrowser(ctx, browserConfig); err != nil { return fmt.Errorf("start browser: %w", err) }
	...
	for _, provider := range rt.ProviderConfigs { providerKeys[provider.ID] = provider.Keys }
	discord.ConfigureModelSettings(&discordtransport.ModelSettingsHandler{
		ResolveSession: sessions.Resolve,
		Providers: rt.Providers,
		ProviderKeys: providerKeys,
		Models: func(ctx context.Context, provider sdk.ProviderID) ([]sdk.Model, error) {
			if err := rt.RefreshProvider(ctx, provider); err != nil { return nil, err }
			return rt.Router.Models(provider), nil
		},
	})
	...
	loop := &sdk.HarnessLoop{
		...
		Source: discord,
		ResolveSession: sessions.Resolve,
		BuildRequest: func(context.Context, sdk.Input, *sdk.Session) (sdk.Request, error) { return sdk.Request{SystemPrompt: os.Getenv("AI_SYSTEM_PROMPT"), MaxOutputTokens: maxOutputTokens}, nil },
		Displays: []sdk.Display{discord},
		DisplayTimeout: 10 * time.Second,
		OnTurnError: func(input sdk.Input, err error) { log.Printf("turn failed source=%s session=%s: %v", input.Source, input.SessionID, err) },
	}
	return loop.Run(ctx)
}
...
func newAgent(client *sdk.RouterClient, workspace string, browser *tools.BrowserClient, allowPrivate bool) (*sdk.Agent, error) {
	...
	return &sdk.Agent{Client: client, Tools: registry, MaxIterations: 20}, nil
}
