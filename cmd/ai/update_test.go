...
func TestInstalledBinaryPreservesSymlinkInvocationPath(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Dir(invoked), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	if got != invoked {
		t.Fatalf("installed binary = %q, want %q", got, invoked)
	}
}
...
func TestReleaseLabel(t *testing.T) {
	if got := releaseLabel(""); got != "latest" {
		t.Fatalf("label = %q", got)
	}
	if got := releaseLabel("v1.29"); got != "v1.29" {
		t.Fatalf("label = %q", got)
	}
}
