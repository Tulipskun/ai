...
`AI_BIN_DIR` can override the executable directory. `AI_DATA_DIR` can override the application state directory. `AI_WORKSPACE` controls the filesystem root used by file, command, and job tools.
