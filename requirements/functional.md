...
REQ-028 — Discord คำสั่ง `/model` ต้องเปิด control panel ในข้อความปกติข้อความเดียว (regular message ไม่ใช่ ephemeral เพราะแก้ไขไม่ได้) แล้วแก้ไขข้อความเดิมทุกครั้งที่มีการกด โดยทุก control บันทึกผลทันที ไม่มี staging ไม่มี modal หนึ่งข้อความมีได้ไม่เกิน 5 action rows ประกอบด้วยแถวปุ่ม `[Main agent] [Sub agent]` (รวมปุ่ม pager `[◀] [(p/n)] [▶]` ต่อท้ายในแถวเดียวกันเมื่อ model catalogue มีหลายหน้า), select menu provider (ไม่ preselect), select menu model แบบแบ่งหน้า 25 รายการต่อหน้า, แถวปุ่ม `[Thinking: ...] [Temp: ...]` ที่กดแล้วแถวเดียวกันกลายเป็น select menu ให้เลือกค่า (thinking: default/none/low/medium/high; temperature: default + 0.0–2.0) และ select menu API pool เมื่อ provider มีรายการเกิน 25 ให้แบ่งหน้าในตัวเลือกเอง (หน้าแรก 24 รายการ + `Next`, หน้าถัดไป `Previous` + รายการ + `Next`) การเปลี่ยน provider ต้องคง model เดิมไว้ถ้ายังอยู่ใน catalogue ไม่เช่นนั้นใช้ model แรกของ catalogue เพื่อให้ session ใช้งานได้เสมอ
