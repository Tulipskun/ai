...
REQ-030 — Session ต้องเก็บ settings แยกกันรายโหมด (provider, model, thinking level, temperature, API key pool index + pool ของแต่ละฝั่ง) แบบ persist ฝั่ง main ใช้ fields เดิม ฝั่ง sub ใช้ `Sub` setters ทั้งหมด (`SetProvider/SetModel/SetTemperature/ClearTemperature/SetThinkingLevel/ClearThinkingLevel/SetKeyIndex`) ต้องเขียนฝั่ง active ตาม `AgentMode` ของ session (default main ทำให้ caller เดิมไม่เปลี่ยนพฤติกรรม) turn ต้องอ่านค่าผ่าน effective config (sub overlay เมื่อ active) ครอบคลุม routing provider/model, thinking/temperature, key pool/index, worker spawn และการเปิด session ใหม่ต้อง re-attach pool ทั้งสองฝั่ง การเข้า sub ครั้งแรกที่ยังไม่มีค่าต้อง seed จากฝั่ง main `/new` ต้องคัดลอกทั้งสองฝั่งพร้อมโหมด
