...
func (c *BrowserConfig) UnmarshalJSON(data []byte) error {
	type raw struct {
		Enabled bool `json:"enabled"`; Mode string `json:"mode"`; Headless bool `json:"headless"`; Browser string `json:"browser"`; Profile string `json:"profile"`; CDPEndpoint string `json:"cdp_endpoint"`; AllowPrivate bool `json:"allow_private"`; IdleTimeout string `json:"idle_timeout"`; NavigationTimeout string `json:"navigation_timeout"`; ActionTimeout string `json:"action_timeout"`; SnapshotTimeout string `json:"snapshot_timeout"`
	}
	var r raw
	...
	idle,err:=parse("idle_timeout",r.IdleTimeout,30*time.Minute);if err!=nil{return err};nav,err:=parse("navigation_timeout",r.NavigationTimeout,30*time.Second);if err!=nil{return err};action,err:=parse("action_timeout",r.ActionTimeout,10*time.Second);if err!=nil{return err};snapshot,err:=parse("snapshot_timeout",r.SnapshotTimeout,10*time.Second);if err!=nil{return err}
	*c=BrowserConfig{Enabled:r.Enabled,Mode:r.Mode,Headless:r.Headless,Browser:r.Browser,Profile:r.Profile,CDPEndpoint:r.CDPEndpoint,AllowPrivate:r.AllowPrivate,IdleTimeout:idle,NavigationTimeout:nav,ActionTimeout:action,SnapshotTimeout:snapshot}
	return nil
}
