...
func TestLoadBrowserConfigDefaults(t *testing.T) {
	...
	if cfg.Enabled || cfg.Mode != "managed" || cfg.Browser != "chromium" || cfg.Profile != "data/browser/profile" || cfg.Headless || cfg.AllowPrivate {
...
func TestPartialBrowserConfigKeepsDefaults(t *testing.T) {
	...
	cfg, err := LoadBrowserConfig(path)
	if err != nil { t.Fatal(err) }
	if !cfg.Enabled || cfg.Mode != "managed" || cfg.Browser != "chromium" || cfg.Profile != "data/browser/profile" || cfg.IdleTimeout != 30*time.Minute || cfg.NavigationTimeout != 30*time.Second || cfg.ActionTimeout != 10*time.Second || cfg.SnapshotTimeout != 10*time.Second { t.Fatalf("partial config lost defaults: %#v", cfg) }
}
