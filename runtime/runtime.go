...
func (r *Runtime) StartBrowser(ctx context.Context, cfg BrowserConfig) error {
	...
	client := tools.NewBrowserClient(tools.BrowserClientConfig{Host: cfg.Host, Port: cfg.Port, NodeCommand: cfg.NodeCommand, WorkerPath: cfg.WorkerPath, WorkerDir: cfg.WorkerDir, Headless: cfg.Headless, AllowPrivate: cfg.AllowPrivate, IdleTimeout: cfg.IdleTimeout, NavigationTimeout: cfg.NavigationTimeout, ActionTimeout: cfg.ActionTimeout, SnapshotTimeout: cfg.SnapshotTimeout, RPCTimeout: cfg.ActionTimeout + 5*time.Second, StartupTimeout: cfg.RPCStartupTimeout})
