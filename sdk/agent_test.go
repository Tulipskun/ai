...
func TestMarkerNudgeContinuesTurn(t *testing.T) {
	...
	if p.calls != 2 {
		t.Fatalf("expected auto-continue, provider calls=%d", p.calls)
	}
	if resp.Content[0].Text != "✨✨✨done" {
		t.Fatalf("final = %q", resp.Content[0].Text)
	}
	h := s.History()
	if len(h) != 4 || h[2].Role != RoleUser {
		t.Fatalf("nudge turn missing: %+v", h)
	}
}
...
func TestMarkerPresentReturnsImmediately(t *testing.T) {
	...
	c, s := newAgentTestSession(p)
	a := &Agent{Client: c, MaxRetries: 0}
	if _, err := a.RunTurn(context.Background(), s, Turn{Role: RoleUser}, Request{}); err != nil {
		t.Fatal(err)
	}
	if p.calls != 1 {
...
func TestMarkerNudgeBounded(t *testing.T) {
	...
	if p.calls != 3 {
		t.Fatalf("provider calls=%d, want 1+2 nudges", p.calls)
	}
	if resp.Content[0].Text != "three" {
		t.Fatalf("final = %q", resp.Content[0].Text)
	}
}
...
func TestHasReplyMarker(t *testing.T) {
	...
	if !HasReplyMarker(marked) {
		t.Fatal("prefixed marker missed")
	}
	plain := Response{Content: []ContentPart{{Type: ContentText, Text: "hi ✨✨✨"}}}
	if HasReplyMarker(plain) {
		t.Fatal("non-prefixed marker should not count")
	}
	if HasReplyMarker(Response{}) {
		t.Fatal("empty response should not count")
	}
}
