...
func (c *Client) headers() map[string]string {
	return map[string]string{"x-api-key": c.APIKey, "anthropic-version": c.APIVersion}
}

func (c *Client) Generate(ctx context.Context, req sdk.Request) (sdk.Response, error) {
	var r response
	err := internal.DoJSON(ctx, c.http(), http.MethodPost, c.BaseURL+"/messages", c.headers(), build(req), &r)
	if err != nil {
	...
	for _, p := range r.Content {
		...
		case "tool_use":
			out.ToolCalls = append(out.ToolCalls, sdk.ToolCall{ID: p.ID, Name: p.Name, Arguments: string(p.Input)})
		}
	}
	return out, nil
}
		...
		err := internal.SSE(ctx, c.http(), http.MethodPost, c.BaseURL+"/messages", c.headers(), build(req), func(data []byte) error {
			...
			case "content_block_start":
				if e.ContentBlock.Type == "tool_use" {
					ch <- sdk.Event{Type: sdk.EventToolCall, ToolCall: &sdk.ToolCall{ID: e.ContentBlock.ID, Name: e.ContentBlock.Name}}
				}
			case "message_stop":
