...
func build(req sdk.Request) map[string]any {
	...
	for _, m := range req.Messages {
		...
		case sdk.RoleToolCall:
			if m.ToolCall != nil {
				toolNames[m.ToolCall.ID] = m.ToolCall.Name
				var args any
				_ = json.Unmarshal([]byte(m.ToolCall.Arguments), &args)
				contents = append(contents, map[string]any{
					"role": "model",
		...
		case sdk.RoleToolResult:
			if m.ToolResult != nil {
				name := toolNames[m.ToolResult.ID]
				if name == "" {
				...
				contents = append(contents, map[string]any{
					...
					"parts": []any{map[string]any{
						"functionResponse": map[string]any{
							"name":     name,
							"response": map[string]any{"content": m.ToolResult.Content},
						},
					}},
				})
			}
	...
	if req.ThinkingLevel != "" && req.ThinkingLevel != sdk.ThinkingNone {
		cfg["thinkingConfig"] = map[string]any{"thinkingLevel": string(req.ThinkingLevel)}
	}
	if len(cfg) > 0 {
...
func (c *Client) Generate(ctx context.Context, req sdk.Request) (sdk.Response, error) {
	...
	return parse(r, req.Model), nil
}

func parse(r response, model string) sdk.Response {
	out := sdk.Response{
		Provider: "gemini",
		Model:    model,
		Usage: sdk.Usage{
		...
		Cache: sdk.CacheInfo{Layer: "provider"},
	}
	out.Cache.Hit = out.Usage.CacheReadTokens > 0
	if len(r.Candidates) == 0 {
		return out
	}
	out.FinishReason = r.Candidates[0].FinishReason
	for _, p := range r.Candidates[0].Content.Parts {
		...
		if p.FunctionCall != nil {
			...
			out.ToolCalls = append(out.ToolCalls, sdk.ToolCall{ID: fmt.Sprintf("gemini-%s-%d", p.FunctionCall.Name, len(out.ToolCalls)+1), Name: p.FunctionCall.Name, Arguments: string(a)})
		}
	}
	return out
}
