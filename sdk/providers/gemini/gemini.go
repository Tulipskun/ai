...
func build(req sdk.Request) map[string]any {
	...
	var contents []any; toolNames:=map[string]string{}
	...
	case sdk.RoleToolCall:
		if m.ToolCall!=nil { toolNames[m.ToolCall.ID]=m.ToolCall.Name; var args any; _=json.Unmarshal([]byte(m.ToolCall.Arguments),&args); contents=append(contents,map[string]any{"role":"model","parts":[]any{map[string]any{"functionCall":map[string]any{"name":m.ToolCall.Name,"args":args}}}}) }
	case sdk.RoleToolResult:
		if m.ToolResult!=nil { name:=toolNames[m.ToolResult.ID]; if name==""{name=m.ToolResult.ID}; contents=append(contents,map[string]any{"role":"user","parts":[]any{map[string]any{"functionResponse":map[string]any{"name":name,"response":map[string]any{"content":m.ToolResult.Content}}}}) }
	} }
...
func parse(r response,model string)sdk.Response{out:=sdk.Response{Provider:"gemini",Model:model,Usage:sdk.Usage{InputTokens:r.Usage.Prompt,OutputTokens:r.Usage.Output,TotalTokens:r.Usage.Total,CacheReadTokens:r.Usage.Cached},Cache:sdk.CacheInfo{Layer:"provider"}};out.Cache.Hit=out.Usage.CacheReadTokens>0;if len(r.Candidates)==0{return out};out.FinishReason=r.Candidates[0].FinishReason;for _,p:=range r.Candidates[0].Content.Parts{if p.Text!=""{out.Content=append(out.Content,sdk.ContentPart{Type:sdk.ContentText,Text:p.Text})};if p.FunctionCall!=nil{a,_:=json.Marshal(p.FunctionCall.Args);out.ToolCalls=append(out.ToolCalls,sdk.ToolCall{ID:fmt.Sprintf("gemini-%s-%d",p.FunctionCall.Name,len(out.ToolCalls)+1),Name:p.FunctionCall.Name,Arguments:string(a)})}};return out}
func(c *Client)Stream(ctx context.Context,req sdk.Request)(<-chan sdk.Event,error){req.Stream=true;ch:=make(chan sdk.Event,16);go func(){defer close(ch);err:=internal.SSE(ctx,c.http(),http.MethodPost,c.endpoint(req.Model,true),c.headers(),build(req),func(data []byte)error{var r response;if json.Unmarshal(data,&r)!=nil{return nil};if len(r.Candidates)==0{return nil};for _,p:=range r.Candidates[0].Content.Parts{if p.Text!=""{ch<-sdk.Event{Type:sdk.EventText,Text:p.Text}};if p.FunctionCall!=nil{a,_:=json.Marshal(p.FunctionCall.Args);ch<-sdk.Event{Type:sdk.EventToolCall,ToolCall:&sdk.ToolCall{ID:fmt.Sprintf("gemini-%s",p.FunctionCall.Name),Name:p.FunctionCall.Name,Arguments:string(a)}}}};if r.Candidates[0].FinishReason!=""{ch<-sdk.Event{Type:sdk.EventDone}};return nil});if err!=nil{ch<-sdk.Event{Type:sdk.EventError,Err:err}}}();return ch,nil}
