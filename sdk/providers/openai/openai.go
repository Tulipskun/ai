...
type response struct {
	...
	Usage struct {
		...
		InputDetails struct { Cached int `json:"cached_tokens"` } `json:"input_tokens_details"`
	} `json:"usage"`
}

func build(req sdk.Request) map[string]any {
	b := map[string]any{"model": req.Model, "stream": req.Stream}
	if req.SystemPrompt != "" { b["instructions"] = req.SystemPrompt }
	var input []any
	for _, m := range req.Messages {
		switch m.Role {
		case sdk.RoleUser, sdk.RoleModel:
			text := ""; for _, p := range m.Content { text += p.Text }
			role := string(m.Role); if m.Role == sdk.RoleModel { role = "assistant" }
	...
	b["input"] = input
	if len(req.Tools) > 0 {
		tools := make([]any, 0, len(req.Tools)); for _, t := range req.Tools { tools = append(tools, map[string]any{"type":"function", "name":t.Name, "description":t.Description, "parameters":t.InputSchema}) }; b["tools"] = tools
	}
	if req.Temperature != nil { b["temperature"] = *req.Temperature }
	if req.MaxOutputTokens > 0 { b["max_output_tokens"] = req.MaxOutputTokens }
	if req.ThinkingLevel != "" && req.ThinkingLevel != sdk.ThinkingNone { b["reasoning"] = map[string]any{"effort":string(req.ThinkingLevel)} }
	return b
}

func parseResponse(r response) sdk.Response {
	...
	for _, item := range r.Output {
		switch item.Type {
		case "message": for _, p := range item.Content { if p.Text != "" { out.Content = append(out.Content, sdk.ContentPart{Type:sdk.ContentText, Text:p.Text}) } }
		case "function_call": out.ToolCalls = append(out.ToolCalls, sdk.ToolCall{ID:item.CallID, Name:item.Name, Arguments:item.Arguments})
		}
	}
...
func (c *Client) Stream(ctx context.Context, req sdk.Request) (<-chan sdk.Event,error) {
	...
	go func(){ defer close(ch); err:=internal.SSE(ctx,c.http(),http.MethodPost,c.BaseURL+"/responses",c.headers(),build(req),func(data []byte)error{
		var e struct{ Type string `json:"type"`; Delta string `json:"delta"`; Item struct{CallID string `json:"call_id"`; Name string `json:"name"`; Arguments string `json:"arguments"`} `json:"item"` }
		if json.Unmarshal(data,&e)!=nil{return nil}; switch e.Type { case "response.output_text.delta": if e.Delta!=""{ch<-sdk.Event{Type:sdk.EventText,Text:e.Delta}}; case "response.function_call_arguments.done": if e.Item.CallID!=""{ch<-sdk.Event{Type:sdk.EventToolCall,ToolCall:&sdk.ToolCall{ID:e.Item.CallID,Name:e.Item.Name,Arguments:e.Item.Arguments}}}; case "response.completed": ch<-sdk.Event{Type:sdk.EventDone} }; return nil
