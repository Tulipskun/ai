...
func (c *RouterClient) RefreshModels(ctx context.Context, provider ProviderID) error {
	config, err := c.Router.Provider(provider); if err != nil { return err }; adapter, ok := c.Adapters[config.Adapter]; if !ok { return &RouteError{Provider: provider, Adapter: config.Adapter} }
	policy := c.Retry; if policy.MaxAttempts <= 0 { policy.MaxAttempts = 1 }; var last error
	for attempt := 1; attempt <= policy.MaxAttempts; attempt++ { err = c.Router.RefreshModels(ctx, provider, adapter); if err == nil { return nil }; last = err; if attempt == policy.MaxAttempts || !retryable(err) { break }; if err := sleepBackoff(ctx, policy, attempt); err != nil { return err } }
	return last
}
...
func (c *RouterClient) generateRetry(ctx context.Context, session *Session, p Provider, req Request) (Response, error) {
	policy := c.Retry; if policy.MaxAttempts <= 0 { policy.MaxAttempts = 1 }; if policy.InitialBackoff <= 0 { policy.InitialBackoff = time.Millisecond }; if policy.MaxBackoff <= 0 { policy.MaxBackoff = policy.InitialBackoff }
	var last error
	for attempt := 1; attempt <= policy.MaxAttempts; attempt++ {
		requestID, recordErr := session.RecordRequest(attempt, req); if recordErr != nil { return Response{}, recordErr }
		resp, err := p.Generate(ctx, req)
		if err == nil {
			if recordErr := session.RecordResponse(requestID, resp, nil); recordErr != nil { return Response{}, recordErr }
			return resp, nil
		}
		last = err
...
// StreamTurn commits streamed output only after EventDone. Any failed stream restores the prior history.
