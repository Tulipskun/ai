...
func (r *Router) RefreshModels(ctx context.Context, provider ProviderID, adapter Provider) error { config, err := r.Provider(provider); if err != nil { return err }; lister, ok := adapter.(ModelLister); if !ok { return fmt.Errorf("sdk: adapter %q does not support model discovery", config.Adapter) }; if config.Keys == nil { return errors.New("sdk: provider has no key pool") }; key, err := config.Keys.Current(); if err != nil { return err }; p := adapter; if kp, ok := p.(KeyedProvider); ok { p = kp.WithAPIKey(key) }; if ep, ok := p.(EndpointProvider); ok && config.BaseURL != "" { p = ep.WithBaseURL(config.BaseURL) }; lister, ok = p.(ModelLister); if !ok { return fmt.Errorf("sdk: configured adapter %q cannot discover models after configuration", config.Adapter) }; models, err := lister.ListModels(ctx, key); if err != nil { return err }; clean := make([]Model, 0, len(models)); seen := make(map[string]struct{}, len(models)); for _, model := range models { if model.ID == "" { continue }; if _, exists := seen[model.ID]; exists { continue }; seen[model.ID] = struct{}{}; clean = append(clean, model) }; r.mu.Lock(); r.catalogs[provider] = clean; r.catalogReady[provider] = true; r.mu.Unlock(); return nil }
