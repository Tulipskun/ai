...
func (s *Session) SetProvider(provider ProviderID, keys *KeyPool) error {
	...
	if provider == "" { return errors.New("sdk: provider is required") }
	if keys == nil { return errors.New("sdk: provider has no key pool") }
	if _, err := keys.At(0); err != nil { return err }
...
func (s *Session) SetModel(model string) error {
	...
	if model == "" { return errors.New("sdk: model is required") }
...
func (s *Session) SetTemperature(temperature float64) error {
	if math.IsNaN(temperature) || math.IsInf(temperature, 0) { return errors.New("sdk: temperature must be finite") }
...
func (s *Session) SetThinkingLevel(level ThinkingLevel) error {
	if !validThinkingLevel(level) { return fmt.Errorf("sdk: invalid thinking level %q", level) }
...
func (s *Session) SetKeyIndex(index int) error {
	if index < 0 { return errors.New("sdk: API key index out of range") }
	if s == nil { return errors.New("sdk: session is nil") }
	s.mu.RLock(); keys := s.keys; s.mu.RUnlock()
	if keys == nil { return errors.New("sdk: session has no key pool") }
	if _, err := keys.At(index); err != nil { return err }
...
func validThinkingLevel(level ThinkingLevel) bool {
	switch level { case ThinkingNone, ThinkingLow, ThinkingMedium, ThinkingHigh: return true; default: return false }
}
func (s *Session) updateConfig(update func(*SessionConfig) error) error {
	if s == nil { return errors.New("sdk: session is nil") }
	s.mu.Lock(); defer s.mu.Unlock()
	config := s.config
	if config.Temperature != nil { v := *config.Temperature; config.Temperature = &v }
	if err := update(&config); err != nil { return err }
	if s.store != nil { if err := s.store.SaveSession(config); err != nil { return err } }
	s.config = config
	return nil
}
