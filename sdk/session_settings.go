...
func (s *Session) SetKeyPool(keys *KeyPool) error {
	...
	s.mu.Lock(); s.keys = keys; s.mu.Unlock()
	return nil
}
...
func (s *Session) SetProvider(provider ProviderID, keys *KeyPool) error {
	...
	if err := s.SetKeyPool(keys); err != nil { return err }
...
func (s *Session) SetAgentMode(mode AgentMode) error { if s == nil { return errors.New("sdk: session is nil") }; switch mode { case AgentModeMain, AgentModeSub: return s.updateConfig(func(config *SessionConfig) error { config.AgentMode = mode; return nil }); default: return fmt.Errorf("sdk: invalid agent mode %q", mode) } }
