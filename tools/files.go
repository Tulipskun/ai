...
func readFileTool(root string) handler {
	return func(_ context.Context, raw json.RawMessage) (string, error) {
		...
		path, err := safePath(root, args.Path); if err != nil { return "", err }
		info, err := os.Stat(path); if err != nil { return "", err }
		if info.IsDir() { return "", errors.New("path is a directory") }
		if info.Size() > maxFileBytes { return "", fmt.Errorf("file exceeds %d byte limit", maxFileBytes) }
...
func writeFileTool(root string) handler {
	return func(_ context.Context, raw json.RawMessage) (string, error) {
		...
		if err := json.Unmarshal(raw, &args); err != nil { return "", err }
		path, err := safePath(root, args.Path); if err != nil { return "", err }
		if int64(len(args.Content)) > maxFileBytes { return "", fmt.Errorf("content exceeds %d byte limit", maxFileBytes) }
		if err := os.MkdirAll(filepath.Dir(path), 0755); err != nil { return "", err }
		if err := os.WriteFile(path, []byte(args.Content), 0644); err != nil { return "", err }
...
func editFileTool(root string) handler {
	return func(_ context.Context, raw json.RawMessage) (string, error) {
		...
		if err := json.Unmarshal(raw, &args); err != nil { return "", err }
		if args.OldText == "" { return "", errors.New("old_text must not be empty") }
		path, err := safePath(root, args.Path); if err != nil { return "", err }
		data, err := os.ReadFile(path); if err != nil { return "", err }
		if int64(len(data)) > maxFileBytes { return "", fmt.Errorf("file exceeds %d byte limit", maxFileBytes) }
		text := string(data)
		count := strings.Count(text, args.OldText)
		if count == 0 { return "", errors.New("old_text was not found") }
		if count != 1 { return "", fmt.Errorf("old_text matched %d times; edit_file requires exactly one match", count) }
		updated := strings.Replace(text, args.OldText, args.NewText, 1)
		if int64(len(updated)) > maxFileBytes { return "", fmt.Errorf("edited file exceeds %d byte limit", maxFileBytes) }
...
func listDirectoryTool(root string) handler {
	return func(_ context.Context, raw json.RawMessage) (string, error) {
		...
		type entry struct { Name string `json:"name"`; Directory bool `json:"directory"` }
		out := make([]entry, 0, len(entries))
		for _, e := range entries { out = append(out, entry{Name: e.Name(), Directory: e.IsDir()}) }
		data, err := json.Marshal(out); return string(data), err
	}
}
...
func searchFilesTool(root string) handler {
	return func(_ context.Context, raw json.RawMessage) (string, error) {
		...
		if err := json.Unmarshal(raw, &args); err != nil { return "", err }
		if args.Query == "" { return "", errors.New("query must not be empty") }
		start, err := safePath(root, args.Path); if err != nil { return "", err }
		matches := make([]string, 0, 16)
		err = filepath.WalkDir(start, func(path string, entry fs.DirEntry, walkErr error) error {
			if walkErr != nil { return walkErr }
			if len(matches) >= maxSearchResults { return fs.SkipAll }
			if entry.IsDir() {
				if path != start && entry.Name() == ".git" { return fs.SkipDir }
				return nil
			}
		...
		data, err := json.Marshal(matches); return string(data), err
	}
}
