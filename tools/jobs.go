...
func NewJobManager(workspace string) *JobManager {
	return &JobManager{workspace: workspace, jobs: make(map[string]*job)}
}
...
func (m *JobManager) Close(id string) error {
	j, err := m.Get(id)
	if err != nil {
		return err
	}
...
func runJobTool(m *JobManager) handler {
	return func(_ context.Context, raw json.RawMessage) (string, error) {
		...
		id, err := m.Start(a.Command, a.Args)
		if err != nil {
			return "", err
		}
...
func closeJobTool(m *JobManager) handler {
	return func(_ context.Context, raw json.RawMessage) (string, error) {
		...
		if err := m.Close(a.JobID); err != nil {
			return "", err
		}
		j, err := m.Get(a.JobID)
		if err != nil {
