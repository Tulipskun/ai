...
func (m *JobManager) Start(command string, args []string) (string, error) {
	...
	j := &job{command: command, args: append([]string(nil), args...), cmd: cmd, state: JobRunning, startedAt: time.Now(), exitCode: -1}
	cmd.Stdout, cmd.Stderr = &j.stdout, &j.stderr
	if err := cmd.Start(); err != nil { return "", err }
	m.mu.Lock(); m.next++; j.id = fmt.Sprintf("job-%d", m.next); m.jobs[j.id] = j; m.mu.Unlock()
	go m.wait(j)
	return j.id, nil
}
...
func (m *JobManager) wait(j *job) {
	err := j.cmd.Wait()
	j.mu.Lock(); defer j.mu.Unlock(); j.finishedAt = time.Now()
	if j.closed { j.state = JobClosed; return }
	if err != nil { j.state = JobFailed; j.err = err.Error(); if exitErr, ok := err.(*exec.ExitError); ok { j.exitCode = exitErr.ExitCode() }; return }
	j.state = JobCompleted; j.exitCode = 0
}
...
func (m *JobManager) Close(id string) error { j, err := m.Get(id); if err != nil { return err }; j.mu.Lock(); if j.state != JobRunning { j.mu.Unlock(); return nil }; j.closed = true; process := j.cmd.Process; j.mu.Unlock(); if process == nil { return nil }; if err := process.Kill(); err != nil && !errors.Is(err, os.ErrProcessDone) { return err }; return nil }
