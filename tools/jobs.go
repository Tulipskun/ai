...
type job struct { mu sync.RWMutex; id, command string; args []string; cmd *exec.Cmd; state JobState; startedAt, finishedAt time.Time; exitCode int; stdout, stderr boundedBuffer; closed bool; err string; done chan struct{} }
...
func (m *JobManager) Close(id string)error{j,err:=m.Get(id);if err!=nil{return err};j.mu.Lock();if j.state!=JobRunning{j.mu.Unlock();return nil};j.closed=true;process:=j.cmd.Process;done:=j.done;j.mu.Unlock();if process==nil{return nil};if err:=process.Kill();err!=nil&&!errors.Is(err,os.ErrProcessDone){return err};<-done;return m.persist()}
