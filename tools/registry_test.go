...
func TestRegistryDefinitions(t *testing.T) {
	...
	defs := r.Definitions()
	if len(defs) != 13 { t.Fatalf("definitions=%d", len(defs)) }
	seen := map[string]bool{}
	for _, d := range defs { seen[d.Name] = true }
	for _, name := range []string{"read_file", "write_file", "edit_file", "list_directory", "search_files", "run_command", "run_job", "check_job", "close_job", "web_fetch", "list_attachments", "read_attachment", "describe_attachment"} { if !seen[name] { t.Fatalf("missing %s", name) } }
}
...
func TestRegistryBrowserDefinitions(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	defs := r.Definitions()
	if len(defs) != 26 { t.Fatalf("definitions=%d", len(defs)) }
	seen := map[string]bool{}
	for _, d := range defs { seen[d.Name] = true }
