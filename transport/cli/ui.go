...
func (u *UI) Trace(event sdk.TraceEvent) {
	...
	case sdk.TraceResponseText:
		...
		if !u.streaming { u.streaming = true; fmt.Fprint(u.Out, "ai › ") }
		fmt.Fprint(u.Out, event.Text)
	case sdk.TraceResponse:
		if u.streaming { return }
		if text := responseText(event.Response); text != "" { fmt.Fprintf(u.Out, "ai › %s", text) }
	case sdk.TraceToolCall:
		if !u.ShowToolTrace || event.ToolCall == nil { return }
		u.endStreamLocked()
		args := Compact(event.ToolCall.Arguments, 420)
		if args == "" { fmt.Fprintf(u.Out, "tool › %s\n", event.ToolCall.Name) } else { fmt.Fprintf(u.Out, "tool › %s %s\n", event.ToolCall.Name, args) }
	case sdk.TraceToolResult:
		if !u.ShowToolTrace || event.ToolResult == nil { return }
		u.endStreamLocked()
		state := "ok"; if event.ToolResult.IsError { state = "error" }
		fmt.Fprintf(u.Out, "tool › %s · %s\n", state, Compact(event.ToolResult.Content, 420))
	case sdk.TraceRetryWait:
		u.endStreamLocked()
		if event.RetryAfter > 0 { fmt.Fprintf(u.Out, "retry › %s in %s\n", CompactError(event.Err), FormatDuration(event.RetryAfter)) } else { fmt.Fprintf(u.Out, "retry › %s\n", CompactError(event.Err)) }
	}
}
...
func ShortID(id string) string { id = strings.TrimSpace(id); if len(id) <= 16 { return id }; return id[len(id)-16:] }
func ModelLabel(provider, model string) string { provider = strings.TrimSpace(provider); model = strings.TrimSpace(model); if provider=="" && model=="" { return "not configured" }; if provider=="" { return model }; if model=="" { return provider+"/<auto>" }; return provider+"/"+model }
