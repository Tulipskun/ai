...
func(d Display)Display(ctx context.Context,output sdk.Output)error{if d.Sender==nil{return errors.New("discord: display has no sender")};channelID:=output.Metadata["channel_id"];if channelID==""{return errors.New("discord: output has no channel_id")};data,err:=json.Marshal(output);if err!=nil{return err};text:=string(data);for _,chunk:=range discordChunks(text,1900){if err:=d.Sender.SendMessage(ctx,channelID,chunk);err!=nil{return err}};return nil}
func(d Display)displayTrace(ctx context.Context,channelID string,trace sdk.TraceEvent)error{sender,hasTrace:=d.Sender.(toolTraceSender);message:=sdk.TraceMessage(trace);switch trace.Stage{case sdk.TraceRequest,sdk.TraceProviderReady,sdk.TraceResponseText,sdk.TraceResponseContent:if !hasTrace{return nil};text:=message;if trace.Text!=""{text += ": "+truncateOneLine(trace.Text,maxToolTraceLength)};if trace.Stage==sdk.TraceResponseText&&trace.Text!=""{text=message+": "+truncateOneLine(trace.Text,maxToolTraceLength)};return sender.appendToolTrace(ctx,channelID,text);case sdk.TraceToolCall:if trace.ToolCall==nil||!hasTrace{return nil};return sender.appendToolTrace(ctx,channelID,formatToolTraceCall(trace.ToolCall));case sdk.TraceToolRunning:if trace.ToolCall==nil||!hasTrace{return nil};return sender.appendToolTrace(ctx,channelID,message+": "+formatToolTraceCall(trace.ToolCall));case sdk.TraceToolResult:if !hasTrace||trace.ToolResult==nil{return nil};return sender.appendToolTrace(ctx,channelID,message+": "+formatToolTraceCall(trace.ToolCall));case sdk.TraceResponse:text:=responseContent(trace.Response);if status,ok:=d.Sender.(retryStatusManager);ok{if err:=status.clearRetryStatus(ctx,channelID);err!=nil{return err}};if text==""{return nil};for _,chunk:=range discordChunks(text,1900){if err:=d.Sender.SendMessage(ctx,channelID,chunk);err!=nil{return err}};return nil;case sdk.TraceRetryWait:var text string;if trace.Err!=nil&&trace.RetryAfter>0{text=fmt.Sprintf("[AI retry] %s\n%s: %s",trace.Err,message,formatDuration(trace.RetryAfter))}else if trace.Err!=nil{text=fmt.Sprintf("[AI retry] %s\n%s",trace.Err,message)};if status,ok:=d.Sender.(retryStatusManager);ok{return status.updateRetryStatus(ctx,channelID,text)};case sdk.TraceError:default:return nil};return nil}
