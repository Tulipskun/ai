...
func(d Display)displayTrace(ctx context.Context,channelID string,trace sdk.TraceEvent)error{sender,hasTrace:=d.Sender.(toolTraceSender);message:=sdk.TraceMessage(trace);switch trace.Stage{
case sdk.TraceRequest:if !hasTrace{return nil};return sender.updateToolTrace(ctx,channelID,message,pendingRequestLine)
...
case sdk.TraceResponse:if hasTrace{if err:=sender.flushToolTrace(ctx,channelID);err!=nil{return err}};if status,ok:=d.Sender.(retryStatusManager);ok{if err:=status.clearRetryStatus(ctx,channelID);err!=nil{return err}};return nil
