...
type Gateway struct { session *discordgo.Session; messages chan InputMessage; done chan struct{}; closeMu sync.Mutex; closed bool; modelSettings *ModelSettingsHandler; providerSettings *ProviderSettingsHandler; stop func(string) bool; retryStatusMu sync.Mutex; retryStatus map[string]string; authorizedUserID string; toolTraceMu sync.Mutex; toolTraceMessage map[string]string }
func NewGateway(token string) (*Gateway, error) { if strings.TrimSpace(token) == "" { return nil, errors.New("discord: bot token is required") }; session, err := discordgo.New("Bot " + token); if err != nil { return nil, err }; gateway := &Gateway{session: session, messages: make(chan InputMessage, 256), done: make(chan struct{}), retryStatus: make(map[string]string), toolTraceMessage: make(map[string]string)}; session.Identify.Intents = gatewayIntents(); session.AddHandler(func(_ *discordgo.Session, event *discordgo.MessageCreate) { if event == nil || event.Author == nil { return }; message, ok := normalizeMessage(discordMessage{ID: event.ID, ChannelID: event.ChannelID, AuthorID: event.Author.ID, AuthorName: event.Author.Username, Content: event.Content, AuthorIsBot: event.Author.Bot}); if !ok { return }; select { case gateway.messages <- message: case <-gateway.done: } }); session.AddHandler(func(s *discordgo.Session, event *discordgo.InteractionCreate) { if event == nil { return }; if gateway.authorizedUserID != "" && event.Member != nil && event.Member.User != nil && event.Member.User.ID != gateway.authorizedUserID { _ = s.InteractionRespond(event.Interaction, &discordgo.InteractionResponse{Type: discordgo.InteractionResponseChannelMessageWithSource, Data: &discordgo.InteractionResponseData{Content: "Unauthorized", Flags: discordgo.MessageFlagsEphemeral}}); return }; if gateway.authorizedUserID != "" && event.Member == nil && event.User != nil && event.User.ID != gateway.authorizedUserID { _ = s.InteractionRespond(event.Interaction, &discordgo.InteractionResponse{Type: discordgo.InteractionResponseChannelMessageWithSource, Data: &discordgo.InteractionResponseData{Content: "Unauthorized", Flags: discordgo.MessageFlagsEphemeral}}); return }; if data, ok := event.Interaction.Data.(discordgo.ApplicationCommandInteractionData); ok && data.Name == "stop" && gateway.stop != nil { content := "ไม่มีงานที่กำลังทำอยู่"; if gateway.stop("discord:channel:" + event.ChannelID) { content = "หยุดการทำงานแล้ว" }; _ = s.InteractionRespond(event.Interaction, &discordgo.InteractionResponse{Type: discordgo.InteractionResponseChannelMessageWithSource, Data: &discordgo.InteractionResponseData{Content: content, Flags: discordgo.MessageFlagsEphemeral}}); return }; if gateway.modelSettings != nil { _ = gateway.modelSettings.Handle(s, event) }; if gateway.providerSettings != nil { _ = gateway.providerSettings.Handle(s, event) } }); return gateway, nil }
...
func (g *Gateway) startToolTrace(ctx context.Context, channelID, content string) error {
	g.toolTraceMu.Lock(); defer g.toolTraceMu.Unlock()
	messageID, err := g.SendStatusMessage(ctx, channelID, content)
	if err != nil { return err }
	g.toolTraceMessage[channelID] = messageID
	return nil
}
func (g *Gateway) finishToolTrace(ctx context.Context, channelID, content string) error {
	g.toolTraceMu.Lock(); defer g.toolTraceMu.Unlock()
	messageID := g.toolTraceMessage[channelID]
	if messageID == "" { return g.SendMessage(ctx, channelID, content) }
	delete(g.toolTraceMessage, channelID)
	return g.EditMessage(ctx, channelID, messageID, content)
}

func (g *Gateway) updateRetryStatus(ctx context.Context, channelID, content string) error { g.retryStatusMu.Lock(); defer g.retryStatusMu.Unlock(); if messageID := g.retryStatus[channelID]; messageID != "" { return g.EditMessage(ctx, channelID, messageID, content) }; messageID, err := g.SendStatusMessage(ctx, channelID, content); if err != nil { return err }; g.retryStatus[channelID] = messageID; return nil }
func (g *Gateway) clearRetryStatus(ctx context.Context, channelID string) error { g.retryStatusMu.Lock(); defer g.retryStatusMu.Unlock(); messageID := g.retryStatus[channelID]; if messageID == "" { return nil }; err := g.DeleteMessage(ctx, channelID, messageID); if err != nil { return err }; delete(g.retryStatus, channelID); return nil }
