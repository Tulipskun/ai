...
type Gateway struct { session *discordgo.Session; messages chan InputMessage; done chan struct{}; closeMu sync.Mutex; closed bool; modelSettings *ModelSettingsHandler; providerSettings *ProviderSettingsHandler }
func NewGateway(token string) (*Gateway, error) {
	...
	gateway := &Gateway{session: session, messages: make(chan InputMessage, 256), done: make(chan struct{})}; session.Identify.Intents = gatewayIntents()
