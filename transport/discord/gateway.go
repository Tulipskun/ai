...
type toolTraceState struct{messageID string;items []string;isText bool;dirty bool;lastPushMs int64;flushTimer *time.Timer;footerActive bool;turnStartMs int64;lastFooterSecMs int64;turnUsage sdk.Usage;footerTimer *time.Timer}
...
func delayToNextSecond()time.Duration{if r:=nowMillis()%1000;r!=0{return time.Duration(1000-r)*time.Millisecond};return time.Second}
func traceFlushDelay(lastPushMs int64)time.Duration{if delayMs:=millisPerSecond-(nowMillis()-lastPushMs);delayMs>0{return time.Duration(delayMs)*time.Millisecond};return 0}
...
func turnFooterText(state *toolTraceState)string{if state==nil||state.turnStartMs==0{return ""};elapsedMs:=nowMillis()-state.turnStartMs;if elapsedMs<0{elapsedMs=0};return "in: "+formatCount(state.turnUsage.InputTokens)+"/"+formatCount(state.turnUsage.CacheReadTokens)+" · out: "+formatCount(state.turnUsage.OutputTokens)+" · ⏱ "+formatElapsed(time.Duration(elapsedMs)*time.Millisecond)}
...
if state.messageID==""{id,err:=g.SendEmbed(ctx,channelID,embed);if err!=nil{return err};state.messageID=id}else if err:=g.EditEmbed(ctx,channelID,state.messageID,embed);err!=nil{if !isUnknownMessage(err){state.dirty=true;state.flushTimer=time.AfterFunc(traceCooldownOnError,func(){if ferr:=g.flushToolTrace(context.Background(),channelID);ferr!=nil{log.Printf("discord: background trace flush failed: %v",ferr)}});return err};id,sendErr:=g.SendEmbed(ctx,channelID,embed);if sendErr!=nil{return sendErr};state.messageID=id}
state.dirty=false;state.lastPushMs=nowMillis();return nil}
...
func(g *Gateway)startTurnFooter(channelID string){if g==nil||strings.TrimSpace(channelID)==""{return};g.toolTraceMu.Lock();defer g.toolTraceMu.Unlock();state:=g.traceState(channelID);if !state.footerActive{state.footerActive=true;state.turnStartMs=nowMillis();state.lastFooterSecMs=0};g.scheduleFooterTickLocked(channelID,state)}
...
func(g *Gateway)scheduleFooterTickLocked(channelID string,state *toolTraceState){if state==nil||!state.footerActive||state.footerTimer!=nil{return};state.footerTimer=time.AfterFunc(delayToNextSecond(),func(){g.tickTurnFooter(channelID)})}
func(g *Gateway)tickTurnFooter(channelID string){if g==nil{return};g.toolTraceMu.Lock();defer g.toolTraceMu.Unlock();state:=g.toolTrace[channelID];if state==nil{return};state.footerTimer=nil;if !state.footerActive{return};sec:=floorSecond(nowMillis());if sec==state.lastFooterSecMs{g.scheduleFooterTickLocked(channelID,state);return};state.lastFooterSecMs=sec;state.dirty=true;g.scheduleTraceFlushLocked(channelID,state);g.scheduleFooterTickLocked(channelID,state)}
