...
type Gateway struct { session *discordgo.Session; messages chan InputMessage; done chan struct{}; closeMu sync.Mutex; closed bool; modelSettings *ModelSettingsHandler; providerSettings *ProviderSettingsHandler; stop func(string) bool; retryStatusMu sync.Mutex; retryStatus map[string]string }
func NewGateway(token string) (*Gateway, error) {
	...
	gateway := &Gateway{session: session, messages: make(chan InputMessage, 256), done: make(chan struct{}), retryStatus: make(map[string]string)}; session.Identify.Intents = gatewayIntents()
	...
	session.AddHandler(func(s *discordgo.Session, event *discordgo.InteractionCreate) { if event == nil { return }; if data, ok := event.Interaction.Data.(discordgo.ApplicationCommandInteractionData); ok && data.Name == "stop" && gateway.stop != nil { content := "ไม่มีงานที่กำลังทำอยู่"; if gateway.stop("discord:channel:" + event.ChannelID) { content = "หยุดการทำงานแล้ว" }; _ = s.InteractionRespond(event.Interaction, &discordgo.InteractionResponse{Type: discordgo.InteractionResponseChannelMessageWithSource, Data: &discordgo.InteractionResponseData{Content: content, Flags: discordgo.MessageFlagsEphemeral}}); return }; if gateway.modelSettings != nil { _ = gateway.modelSettings.Handle(s, event) }; if gateway.providerSettings != nil { _ = gateway.providerSettings.Handle(s, event) } })
	return gateway, nil
}
...
func (g *Gateway) Start(ctx context.Context) error {
	...
	if g.modelSettings != nil { if err := g.registerCommand(&discordgo.ApplicationCommand{Name: "model", Description: "Configure the AI model for this session"}); err != nil { return err } }
	if g.providerSettings != nil { if err := g.registerCommand(&discordgo.ApplicationCommand{Name: "provider", Description: "Add or update an AI provider"}); err != nil { return err } }
	if g.stop != nil { if err := g.registerCommand(&discordgo.ApplicationCommand{Name: "stop", Description: "Stop the current AI task"}); err != nil { return err } }
