...
type Gateway struct { session *discordgo.Session; messages chan InputMessage; done chan struct{}; closeMu sync.Mutex; closed bool; modelSettings *ModelSettingsHandler; providerSettings *ProviderSettingsHandler; retryStatusMu sync.Mutex; retryStatus map[string]string }
func NewGateway(token string) (*Gateway, error) {
	...
	session.AddHandler(func(s *discordgo.Session, event *discordgo.InteractionCreate) { if gateway.modelSettings != nil { _ = gateway.modelSettings.Handle(s, event) }; if gateway.providerSettings != nil { _ = gateway.providerSettings.Handle(s, event) } })
	return gateway, nil
}
