...
func normalizeMessage(message discordMessage) (InputMessage, bool) {
	if message.AuthorIsBot || message.ID == "" || message.ChannelID == "" || message.AuthorID == "" { return InputMessage{}, false }
...
func NewGateway(token string) (*Gateway, error) {
	...
	session.AddHandler(func(_ *discordgo.Session, event *discordgo.MessageCreate) {
		if event == nil || event.Author == nil { return }
		...
		if !ok { return }
		select { case gateway.messages <- message: case <-gateway.done: }
	})
	session.AddHandler(func(s *discordgo.Session, event *discordgo.InteractionCreate) {
		if gateway.modelSettings == nil { return }
		if err := gateway.modelSettings.Handle(s, event); err != nil { _ = gateway.SendMessage(context.Background(), event.ChannelID, "Model settings error: "+err.Error()) }
	})
	return gateway, nil
}
...
func (g *Gateway) Start(ctx context.Context) error {
	...
	if g.modelSettings != nil {
		_, err := g.session.ApplicationCommandCreate(g.session.State.User.ID, "", &discordgo.ApplicationCommand{Name: "model", Description: "Configure the AI model for this session"})
