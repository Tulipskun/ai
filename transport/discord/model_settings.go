...
func (h *ModelSettingsHandler) Handle(s *discordgo.Session, i *discordgo.InteractionCreate) error {
	if h == nil || i == nil || i.Type != discordgo.InteractionApplicationCommand && i.Type != discordgo.InteractionMessageComponent {
		return nil
	}
...
func (h *ModelSettingsHandler) respondSettings(s *discordgo.Session, i *discordgo.InteractionCreate, channelID string) error {
	...
	input := sdk.Input{SessionID: "discord:channel:" + channelID}
	session, err := h.ResolveSession(context.Background(), input)
	if err != nil {
	...
	return s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{
		...
		Data: &discordgo.InteractionResponseData{Content: h.summary(session), Flags: discordgo.MessageFlagsEphemeral, Components: components},
	})
}
...
func (h *ModelSettingsHandler) handleSelection(s *discordgo.Session, i *discordgo.InteractionCreate, customID string) error {
	...
	field, sessionID, value := parts[1], parts[2], parts[3]
	if h.ResolveSession == nil {
		return h.respondError(s, i, "session manager is not configured")
	}
	session, err := h.ResolveSession(context.Background(), sdk.Input{SessionID: sessionID})
	if err != nil {
		return h.respondError(s, i, err.Error())
	}
	switch field {
	case "provider":
		keys := h.ProviderKeys[sdk.ProviderID(value)]
		err = session.SetProvider(sdk.ProviderID(value), keys)
	case "model":
	...
	case "temperature":
		...
		if err == nil {
			err = session.SetTemperature(temperature)
		}
	case "key":
		var index int
...
func (h *ModelSettingsHandler) components(ctx context.Context, session *sdk.Session) ([]discordgo.MessageComponent, error) {
	...
	rows := []discordgo.MessageComponent{
		discordgo.ActionsRow{Components: []discordgo.MessageComponent{h.selectMenu("provider", "Provider", string(config.Provider), session.ID(), providerOptions(h.Providers))}},
	}
	models := []sdk.Model{}
	...
	if len(modelOptions) == 0 && config.Model != "" {
		modelOptions = append(modelOptions, discordgo.SelectMenuOption{Label: truncate(config.Model, 100), Value: config.Model, Default: true})
	}
	...
	for n := 0; n <= 10; n++ {
		value := strconv.FormatFloat(float64(n)/10, 'f', 1, 64)
		defaultValue := config.Temperature != nil && *config.Temperature == float64(n)/10
		temperatureOptions = append(temperatureOptions, discordgo.SelectMenuOption{Label: value, Value: value, Default: defaultValue})
	}
...
func (h *ModelSettingsHandler) selectMenu(field, placeholder, value, sessionID string, options []discordgo.SelectMenuOption) discordgo.SelectMenu {
	...
	return discordgo.SelectMenu{CustomID: "model:" + field + ":" + sessionID + ":" + value, Placeholder: placeholder, Options: options, MinValues: 1, MaxValues: 1}
}
...
func providerOptions(providers []sdk.ProviderID) []discordgo.SelectMenuOption {
	...
	for _, provider := range providers { options = append(options, discordgo.SelectMenuOption{Label: string(provider), Value: string(provider)}) }
	return options
}
...
func (h *ModelSettingsHandler) summary(session *sdk.Session) string {
	...
	return fmt.Sprintf("**Session Model Settings**\nProvider: `%s`\nModel: `%s`\nThinking: `%s`\nTemperature: `%s`\nAPI Pool: `%d`", config.Provider, model, thinking, temperature, config.KeyIndex+1)
}
