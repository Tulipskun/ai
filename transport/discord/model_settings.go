...
func (h *ModelSettingsHandler) Handle(s *discordgo.Session, i *discordgo.InteractionCreate) error {
	...
	if i.Type == discordgo.InteractionMessageComponent {
		...
		if customID == modelSettingsProviderSelect { return h.handleProviderSelect(s, i) }
		if strings.HasPrefix(customID, modelSettingsModelSelectPrefix) { return h.handleModelSelect(s, i, customID) }
		return nil
	}
	if i.Type == discordgo.InteractionModalSubmit {
		customID := i.ModalSubmitData().CustomID
		if strings.HasPrefix(customID, modelSettingsPrefix) { return h.handleSettingsSubmit(s, i, customID) }
	}
	return nil
}
...
func (h *ModelSettingsHandler) respondProviderStep(s *discordgo.Session, i *discordgo.InteractionCreate, channelID string) error {
	...
	return s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{Type: discordgo.InteractionResponseChannelMessageWithSource, Data: providerSelectionMessage(channelID, provider, h.Providers)})
}

func providerSelectionModal(channelID, current string, providers []sdk.ProviderID) *discordgo.InteractionResponseData {
	return &discordgo.InteractionResponseData{CustomID: modelSettingsProviderStep + channelID, Title: "Model Settings — Step 1", Components: []discordgo.MessageComponent{
...
func providerSelectionMessage(channelID, current string, providers []sdk.ProviderID) *discordgo.InteractionResponseData {
	return &discordgo.InteractionResponseData{Content: "Select a provider:", Flags: discordgo.MessageFlagsEphemeral, Components: []discordgo.MessageComponent{discordgo.ActionsRow{Components: []discordgo.MessageComponent{
...
func (h *ModelSettingsHandler) handleProviderSelect(s *discordgo.Session, i *discordgo.InteractionCreate) error {
	...
	if len(values) == 0 || strings.TrimSpace(values[0]) == "" { return h.respondError(s, i, "provider is required") }
	...
	if h.ProviderKeys[provider] == nil { return h.respondError(s, i, fmt.Sprintf("provider %q has no API key pool", provider)) }
	if h.Models == nil { return h.respondError(s, i, "model catalogue loader is not configured") }
	models, err := h.Models(context.Background(), provider)
	if err != nil { return h.respondError(s, i, err.Error()) }
	if len(models) == 0 { return h.respondError(s, i, fmt.Sprintf("provider %q has no models", provider)) }
	groups := makeModelOptionGroups(models, "")
	if len(groups) == 0 { return h.respondError(s, i, fmt.Sprintf("provider %q has no usable models", provider)) }
	firstEnd := minInt(5, len(groups))
	if err := s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{Type: discordgo.InteractionResponseChannelMessageWithSource, Data: &discordgo.InteractionResponseData{Content: fmt.Sprintf("Select a model for `%s`:", provider), Flags: discordgo.MessageFlagsEphemeral, Components: makeModelSelectionComponents(i.ChannelID, string(provider), groups[:firstEnd], 0)}}); err != nil { return err }
	for start := 5; start < len(groups); start += 5 {
		end := start + 5; if end > len(groups) { end = len(groups) }
		if _, err := s.FollowupMessageCreate(i.Interaction, true, &discordgo.WebhookParams{Content: "More models:", Flags: discordgo.MessageFlagsEphemeral, Components: makeModelSelectionComponents(i.ChannelID, string(provider), groups[start:end], start)}); err != nil { return err }
	}
	return nil
}
...
func (h *ModelSettingsHandler) handleModelSelect(s *discordgo.Session, i *discordgo.InteractionCreate, customID string) error {
	...
	if len(parts) != 3 || parts[0] == "" || parts[1] == "" || parts[2] == "" { return h.respondError(s, i, "invalid model selection") }
	...
	if h.Models != nil {
		...
		if modelErr != nil { return h.respondError(s, i, modelErr.Error()) }
		if !modelInCatalog(models, model) { return h.respondError(s, i, fmt.Sprintf("model %q is not available for provider %q", model, provider)) }
	}
	if err := session.SetProvider(provider, h.ProviderKeys[provider]); err != nil { return h.respondError(s, i, err.Error()) }
...
func modelSettingsModal(sessionID, provider, model, temperature, thinking, key string, keyCount ...int) *discordgo.InteractionResponseData {
	resolvedKeyCount := 1; if len(keyCount) > 0 && keyCount[0] > 0 { resolvedKeyCount = keyCount[0] }
	channelID := strings.TrimPrefix(sessionID, "discord:channel:")
	return &discordgo.InteractionResponseData{CustomID: modelSettingsPrefix + channelID + ":" + provider, Title: "Model Settings — Step 2", Components: []discordgo.MessageComponent{
		...
		discordgo.Label{Label: "Temperature", Description: "Enter a number from 0.0 to 2.0. Leave blank for default.", Component: discordgo.TextInput{CustomID: "temperature", Style: discordgo.TextInputShort, Value: normalizeTemperatureInput(temperature), Placeholder: "0.0 - 2.0", Required: boolPtr(false), MaxLength: 20}},
...
func modelSettingsModalWithCatalog(sessionID, provider, model, temperature, thinking, key string, models []sdk.Model, keyCount int) *discordgo.InteractionResponseData { return modelSettingsModal(sessionID, provider, model, temperature, thinking, key, keyCount) }
func makeProviderOptions(providers []sdk.ProviderID, current string) []discordgo.SelectMenuOption { options:=make([]discordgo.SelectMenuOption,0,minInt(len(providers),25));for _,provider:=range providers{value:=string(provider);if value==""||len(options)>=25{continue};options=append(options,discordgo.SelectMenuOption{Label:value,Value:value,Default:false})};return options }
func makeModelOptions(models []sdk.Model, current string) []discordgo.SelectMenuOption { groups:=makeModelOptionGroups(models,current);if len(groups)==0{return nil};return groups[0] }
...
func makeModelSelectionComponents(channelID,provider string,groups [][]discordgo.SelectMenuOption,groupOffset int) []discordgo.MessageComponent {components:=make([]discordgo.MessageComponent,0,minInt(len(groups),5));for index,options:=range groups{groupIndex:=groupOffset+index;start:=groupIndex*25+1;end:=start+len(options)-1;customID:=fmt.Sprintf("%s%s:%s:%d",modelSettingsModelSelectPrefix,channelID,provider,groupIndex);label:=fmt.Sprintf("Models %d-%d",start,end);components=append(components,discordgo.ActionsRow{Components:[]discordgo.MessageComponent{discordgo.SelectMenu{CustomID:customID,MenuType:discordgo.StringSelectMenu,Placeholder:label,Options:options,MinValues:intPtr(1),MaxValues:1}}})};return components }
...
func (h *ModelSettingsHandler) handleSettingsSubmit(s *discordgo.Session, i *discordgo.InteractionCreate, customID string) error {
	value := strings.TrimPrefix(customID, modelSettingsPrefix); parts := strings.SplitN(value, ":", 2)
	...
	if temperatureText != "" && temperatureText != "default" { value, parseErr := strconv.ParseFloat(temperatureText, 64); if parseErr != nil || value < 0 || value > 2 { return h.respondError(s, i, "temperature must be a number from 0.0 to 2.0") }; temperature = &value }
	if thinking != "" && thinking != "default" && !validDiscordThinkingLevel(thinking) { return h.respondError(s, i, fmt.Sprintf("invalid thinking level %q", thinking)) }
	keyIndex, parseErr := strconv.Atoi(keyText); if parseErr != nil || keyIndex < 1 { return h.respondError(s, i, "API Pool must be a positive number") }; keyIndex--; if _, err := keys.At(keyIndex); err != nil { return h.respondError(s, i, err.Error()) }
	if err := session.SetProvider(provider, keys); err != nil { return h.respondError(s, i, err.Error()) }; if err := session.SetModel(model); err != nil { return h.respondError(s, i, err.Error()) }
	if temperature == nil { if err := session.ClearTemperature(); err != nil { return h.respondError(s, i, err.Error()) } } else if err := session.SetTemperature(*temperature); err != nil { return h.respondError(s, i, err.Error()) }
	if thinking == "" || thinking == "default" { if err := session.ClearThinkingLevel(); err != nil { return h.respondError(s, i, err.Error()) } } else if err := session.SetThinkingLevel(sdk.ThinkingLevel(thinking)); err != nil { return h.respondError(s, i, err.Error()) }
	if err := session.SetKeyIndex(keyIndex); err != nil { return h.respondError(s, i, err.Error()) }
	return s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{Type: discordgo.InteractionResponseChannelMessageWithSource, Data: &discordgo.InteractionResponseData{Content: h.summary(session), Flags: discordgo.MessageFlagsEphemeral}})
}
...
func(h *ModelSettingsHandler)summary(session *sdk.Session)string{config:=session.Config();thinking:=string(config.ThinkingLevel);if thinking==""{thinking="default"};model:=config.Model;if model==""{model="not set"};return fmt.Sprintf("**Session Model Settings**\nProvider: `%s`\nModel: `%s`\nThinking: `%s`\nTemperature: `%s`\nAPI Pool: `%d`",config.Provider,model,thinking,temperatureLabel(config.Temperature),config.KeyIndex+1)}
func temperatureLabel(value *float64)string{if value==nil{return "default"};return strconv.FormatFloat(*value,'f',1,64)}
func boolPtr(value bool)*bool{return &value};func intPtr(value int)*int{return &value};func minInt(a,b int)int{if a<b{return a};return b}
