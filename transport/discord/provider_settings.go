...
func (h *ProviderSettingsHandler) Handle(s *discordgo.Session, i *discordgo.InteractionCreate) error {
	...
	if i.Type == discordgo.InteractionApplicationCommand {
		if i.ApplicationCommandData().Name != "provider" {
			return nil
		}
		return h.openModal(s, i)
	}
	if i.Type != discordgo.InteractionModalSubmit || i.ModalSubmitData().CustomID != providerSettingsModalID {
		return nil
	}
	return h.submit(s, i)
}
...
func (h *ProviderSettingsHandler) openModal(s *discordgo.Session, i *discordgo.InteractionCreate) error {
	if len(h.Adapters) == 0 {
		return respondProviderError(s, i, "no adapters are available")
	}
...
func (h *ProviderSettingsHandler) submit(s *discordgo.Session, i *discordgo.InteractionCreate) error {
	if h.Upsert == nil {
		return respondProviderError(s, i, "provider manager is not configured")
	}
	values := modalValues(i)
	name := strings.TrimSpace(values["name"])
	adapter := strings.TrimSpace(strings.ToLower(values["adapter"]))
	endpoint := strings.TrimSpace(values["url"])
	apiKey := strings.TrimSpace(values["api_key"])
	if name == "" || adapter == "" || endpoint == "" || apiKey == "" {
		return respondProviderError(s, i, "name, adapter, URL, and API key are required")
	}
	if err := h.Upsert(context.Background(), name, adapter, endpoint, apiKey); err != nil {
		return respondProviderError(s, i, err.Error())
	}
	return s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{
		Type: discordgo.InteractionResponseChannelMessageWithSource,
		Data: &discordgo.InteractionResponseData{Content: fmt.Sprintf("Provider `%s` saved and model catalogue refreshed.", name), Flags: discordgo.MessageFlagsEphemeral},
	})
}
...
func respondProviderError(s *discordgo.Session, i *discordgo.InteractionCreate, message string) error {
	return s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{
		Type: discordgo.InteractionResponseChannelMessageWithSource,
		Data: &discordgo.InteractionResponseData{Content: "Provider settings error: " + message, Flags: discordgo.MessageFlagsEphemeral},
	})
}
